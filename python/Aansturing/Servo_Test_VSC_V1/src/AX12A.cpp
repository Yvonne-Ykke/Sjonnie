...
void AX12A::begin(long baud, unsigned char directionPinTX, unsigned char directionPinRX, unsigned char directionPinTR, HardwareSerial *srl)
{
	varSerial = srl;
	Direction_Pin_TX = directionPinTX;
	Direction_Pin_RX = directionPinRX;
	Direction_Pin_TR = directionPinTR;
	setDPin(Direction_Pin_TX, OUTPUT);
	setDPin(Direction_Pin_RX, INPUT);
	setDPin(Direction_Pin_TR, OUTPUT);
...
{
	switchCom(Direction_Pin_TR, TX_MODE); 	// Switch to Transmission  Mode

	sendData(packet, length);			// Send data through sending buffer
	flush(); 							// Wait until buffer is empty

	switchCom(Direction_Pin_TR, RX_MODE); 	// Switch back to Reception Mode
